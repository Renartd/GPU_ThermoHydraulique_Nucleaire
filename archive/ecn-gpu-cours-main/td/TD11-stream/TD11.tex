...
%%%%%%%%%%%%%%%%%%%%%%%%%
  \section{Addition vectorielle avec des streams CUDA}
%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{objectif}[]{}
 \begin{itemize}
	\item se familiariser avec l'utilisation de l'API de streaming CUDA
\end{itemize}
\end{objectif}

\begin{wtodo}[Addition vectorielle avec streams]{}

Réimplémentez l'addition de vecteurs en utilisant les streams CUDA.

Utilisez le code fourni comme base pour implémenter votre code avec des streams. Vous devez :
\begin{itemize}
	\item Utiliser au moins 4 flux CUDA dans votre programme, mais vous pouvez le rendre paramètrable pour les plus grands ensembles de données
	\item Choisir une taille de segment significative (si le segment est trop petit il n'y aura pas de gain en terme de temps de chargement mémoire) et de manière à être un multiple de la taille des données fois le nombre de streams (pour ne pas gérer des conditions de terminaison compliquées)
	\item Allouer pour chaque stream la mémoire du device 
	\item Entrelacer les copies de la mémoire asyncrhones
	\item Invoquer le kernel CUDA dans les streams
	\item Copier les résultats du device sur l'hôte de manière asynchrone
\end{itemize}

Comparez les performances obtenues avec la version sans streams.
\end{wtodo}
