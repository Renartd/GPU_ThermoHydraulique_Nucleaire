...
struct SimPanel {
    bool visible  = false;
    char inputBuf[16] = "1";   // buffer saisie vitesse
    bool inputFocus = false;   // champ actif ?

    // Applique la valeur saisie à speedFactor
    void applyInput(SimControl& ctrl) {
        float val = (float)atof(inputBuf);
        if (val >= 0.125f && val <= 1000.0f)
            ctrl.speedFactor = val;
        // Remet le buffer à la valeur courante
    ...
    bool update(SimControl& ctrl, int sw, int sh) {
        ...
        if (radioBtn("Uniforme", ctrl.fluxMode == FluxMode::UNIFORME, PX+170, y))
        ...
        DrawText("-", (int)btnMinus.x+10, (int)btnMinus.y+4, 14, WHITE);
        DrawText("+", (int)btnPlus.x+9,  (int)btnPlus.y+4,  14, WHITE);
        if (clicked && hoverM) {
